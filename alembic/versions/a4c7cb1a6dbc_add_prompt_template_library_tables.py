...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "style_keywords", postgresql.ARRAY(sa.String()), server_default=sa.text("'{}'::varchar[]"), nullable=False
        ),
        sa.Column(
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_prompt_templates_category", table_name="prompt_templates", postgresql_where=sa.text("deleted_at IS NULL")
    )
