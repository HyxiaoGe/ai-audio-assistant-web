...
def upgrade() -> None:
    ...
    op.create_index("ix_transcripts_task_sequence", "transcripts", ["task_id", "sequence"], unique=False)

    # Summaries table index for task lookup
    op.create_index("ix_summaries_task_type", "summaries", ["task_id", "summary_type", "is_active"], unique=False)
